...
export function InsectQuiz() {
  ...
  const [seenObservationIds, setSeenObservationIds] = useState<number[]>([]);
  ...
  const loadNewQuestion = async () => {
    ...
    try {
      const question = await fetchQuizQuestion(
        ...
        seenObservationIds.length > 0 ? seenObservationIds : undefined
      );
      setSeenObservationIds(prev => [...prev, question.insect.id]);
      setState(prev => ({
        ...prev,
  ...
  const handleFiltersApply = (countryId: number, taxons: string[], dateFrom: string, dateTo: string) => {
    setSelectedCountryId(countryId);
    setSelectedTaxons(taxons);
    setSelectedDateFrom(dateFrom);
    setSelectedDateTo(dateTo);
    setSeenObservationIds([]);
  };
